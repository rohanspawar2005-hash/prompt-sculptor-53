...
const ProjectCard = ({ project: p }: { project: Project }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-lg overflow-hidden border border-white/[0.06] bg-white/[0.02] cursor-pointer shadow-[0_0_0_1px_rgba(255,255,255,0.02)] transition-colors duration-500 hover:border-gold/35 hover:bg-white/[0.035] hover:shadow-[0_30px_60px_-30px_rgba(200,168,122,0.35)]"
    >
      {/* Image Container */}
      <div
        className={`relative aspect-[16/10] overflow-hidden ${p.imgClass?.includes("bg-") ? p.imgClass.split(" ").filter((c) => c.startsWith("bg-")).join(" ") : "bg-white/[0.03]"}`}
        style={{ transform: "translateZ(40px)" }}
      >
        {/* Mouse-tracked glare */}
        <motion.div
          className="pointer-events-none absolute inset-0 z-20 opacity-0 group-hover:opacity-100 mix-blend-overlay transition-opacity duration-500"
          style={{
            background: useTransform(
              [glareX, glareY] as never,
              ([x, y]: [string, string]) =>
                `radial-gradient(circle at ${x} ${y}, rgba(255,255,255,0.55), rgba(255,255,255,0) 45%)`
            ),
          }}
        />
        {/* Shimmer sweep */}
        <motion.div
          ...
          animate={{ backgroundPosition: hovered ? "140% 0%" : "-40% 0%" }}
          transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}
          style={{
            backgroundImage:
              "linear-gradient(105deg, transparent 34%, rgba(200,168,122,0.2) 48%, transparent 62%)",
        ...
        {/* Project number */}
        <span className="absolute top-4 left-4 text-[11px] font-bold tracking-[2px] text-white/40 uppercase transition-colors duration-300 group-hover:text-gold" style={{ transform: "translateZ(30px)" }}>
          {p.num}
        </span>
      ...
      <div className="p-5 md:p-6" style={{ transform: "translateZ(20px)" }}>
        ...
        <p className="text-[12px] text-text-primary/60 leading-[1.7] mb-4 line-clamp-2">{p.desc}</p>
